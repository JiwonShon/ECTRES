...
out_prefix      <- "EFM19_ecDNA_parental"
# ============================================================================
...
junctions <- dat %>%
  ...
  mutate(
    VAF = AD / (AD + DP),
    unreliable = !is.na(AD) & !is.na(DP) & (AD > DP),
    is_ecdna = grepl("ecDNA-like", AA_CYCLECLASS, fixed = TRUE)
  )

# ---------------------------------------------------------------------------
# 3. clone junction을 parental 좌표 기준으로 reconcile (window 내 최근접, orientation 무관)
# ---------------------------------------------------------------------------
...
match_to_ref <- function(row, ref, window) {
  same_fwd <- ref$chrom1 == row$chrom1 & ref$chrom2 == row$chrom2
  same_rev <- ref$chrom1 == row$chrom2 & ref$chrom2 == row$chrom1
  cand <- ref[same_fwd | same_rev, ]
  if (nrow(cand) == 0) return(NA_character_)
  d_fwd <- abs(cand$pos1 - row$pos1) + abs(cand$pos2 - row$pos2)
  d_rev <- abs(cand$pos1 - row$pos2) + abs(cand$pos2 - row$pos1)
  d <- pmin(d_fwd, d_rev)
  best <- which.min(d)
...
if (n_novel > 0) {
  message(sprintf("[note] %d개 clone junction이 parental 기준 %dbp 이내에서 매칭 안 됨 -- 그림에서 제외 (clone-specific SV 후보). window_bp를 늘려야 할 수도 있음.",
                   n_novel, window_bp))
}

matched <- junctions %>% filter(!is.na(ref_junction_id))

bp_label_map <- ref %>% mutate(bp_label = sprintf("%s:%s-%s:%s", chrom1, format(pos1, big.mark=","),
                                                    chrom2, format(pos2, big.mark=","))) %>%
  select(ref_junction_id, bp_label)
matched <- matched %>% left_join(bp_label_map, by = "ref_junction_id")

# ---------------------------------------------------------------------------
# 4. matrix 구성: VAF, ecDNA flag, unreliable flag (row=bp_label, col=sample)
# ---------------------------------------------------------------------------
samples <- unique(mapping_sub$sample_id)
samples <- c("parental", setdiff(samples, "parental"))
bp_labels <- bp_label_map$bp_label
...
unrel_mat <- to_matrix("unreliable"); unrel_mat[is.na(unrel_mat)] <- FALSE
...
unrel_mat <- unrel_mat[, sample_order, drop = FALSE]
...
cell_fun <- function(j, i, x, y, width, height, fill) {
  ...
  if (is.na(v)) {
    ...
    label <- sprintf("%.2f", v)
    if (unrel_mat[i, j]) label <- paste0(label, "*")   # AD>DP -- 신뢰 낮음 표시
...
ht2 <- Heatmap(vaf_mat,
               ...
               column_title = sprintf(
                 "%s-linked SV breakpoints: parental vs clones\n(cell = VAF = AD/(AD+DP); red box = ecDNA-defining SV (AA_CYCLECLASS=ecDNA-like); * = AD>DP, VAF unreliable)",
                 source_barcode),
               row_names_side = "left",
               column_names_rot = 90,
               rect_gp = gpar(col = "white", lwd = 0.5),
...
message(sprintf("Samples: %d (parental + %d clones) | Breakpoints (parental-defined): %d | Novel clone-only junctions excluded: %d",
                 length(sample_order), length(clone_order), nrow(vaf_mat), n_novel))