...
source_barcode  <- "ECGI1"     # 이 source(환자)에 속한 샘플들만
...
if (reference_mode == "union") {
  ...
  rep_tbl <- junctions %>%
    ...
    mutate(bp_label = sprintf("%s:%s-%s:%s", rep_chrom1, format(rep_pos1, big.mark = ","),
                               rep_chrom2, format(rep_pos2, big.mark = ","))) %>%
    arrange(rep_chrom1, rep_pos1)

  matched <- junctions %>% left_join(rep_tbl %>% select(ref_junction_id, bp_label), by = "ref_junction_id")
